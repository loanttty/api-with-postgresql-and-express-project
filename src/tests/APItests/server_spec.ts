...
describe("Test Endpoints /product/:id", () => {
  it("should response error code 404 if no product with same id is found", (done) => {
    supertest(app).get("/product/4").expect(404, done);
  });
});
...
describe("Test Endpoints /products-by-category/:category", () => {
  it("should response error code 404 'Not Found'", (done) => {
    supertest(app).get("/products-by-category/Desktop").expect(404, done);
  });
});
describe("Test Endpoints /top-five-products", () => {
  it("should response error code 404 'Not Found'", (done) => {
    supertest(app).get("/top-five-products").expect(404, done);
  });
});
